...
async function runTests() {
  try {
    ...
    console.log("\n--- [Test 13] Workflow Multi-Tool: Retrieval + Unit Converter ---");
    {
      ...
      workflowGraphService.setAgentBrainLlmClient(async () => {
        ...
        if (stepCount === 1) {
          return JSON.stringify({
            ...
            action: "tool",
            tool: "retrieve_information",
            reason: "Look up discharge pressure of pump P-204.",
        ...
        if (stepCount === 2) {
          return JSON.stringify({
            ...
            action: "tool",
            tool: "unit_converter",
            reason: "Convert retrieved discharge pressure (7.2 bar) to psi.",
        ...
        return JSON.stringify({
          ...
          answer: "Discharge pressure of P-204 is 7.2 bar, which is approximately 104.43 psi.",
        });
      });

      const mockRetriever = async () => ({
        results: [{ filename: "P204_Specs.pdf", page: 1, text: "Discharge pressure = 7.2 bar" }],
      });

      const res = await runWorkflowTask({
        message: "What is the discharge pressure of pump P-204 in psi?",
        userId: "test-eng-1",
        options: { retriever: mockRetriever },
      });
      ...
      assert.strictEqual(res.steps.length, 2);
      assert.strictEqual(res.steps[0].toolName, "retrieve_information");
      assert.strictEqual(res.steps[1].toolName, "unit_converter");
      assert.ok(res.response.includes("104.43"));
      pass("Test 13 passed: Retrieval + Unit Converter task correctly chains Retrieval → Unit Converter");
    }
    ...
    console.log("\n--- [Test 14] Workflow Multi-Tool: Retrieval + Calculator + Unit Converter ---");
    {
      ...
      workflowGraphService.setAgentBrainLlmClient(async () => {
        ...
        if (stepCount === 1) {
          return JSON.stringify({
            ...
            input: { query: "P-204 pressure readings" },
          });
        }
        if (stepCount === 2) {
          return JSON.stringify({
            ...
            action: "tool",
            tool: "calculator",
            reason: "Calculate differential pressure: 7.2 - 1.8.",
        ...
        return JSON.stringify({
          workflow: "general",
          action: "final",
          reason: "Calculation and unit conversion complete.",
          answer: "The pressure differential is 5.4 bar (7.2 - 1.8), which converts to 78.32 psi.",
        });
      });

      const mockRetriever = async () => ({
        results: [{ filename: "P204_Report.pdf", text: "Suction: 1.8 bar, Discharge: 7.2 bar" }],
      });

      const res = await runWorkflowTask({
        message: "Calculate the pressure differential of P-204 and provide it in psi.",
        userId: "test-eng-1",
        options: { retriever: mockRetriever },
      });
      ...
      pass("Test 14 passed: Multi-tool task correctly chains Retrieval → Calculator → Unit Converter");
    }
    ...
    console.log("\n--- [Test 15] Workflow Multi-Tool: Vision + Calculator ---");
    {
      ...
      workflowGraphService.setAgentBrainLlmClient(async () => {
        ...
        if (stepCount === 1) {
          return JSON.stringify({
            ...
            action: "tool",
            tool: "vision",
            reason: "Extract tank level percentages from the gauge photo.",
            input: { prompt: "Read the primary and reserve tank percentage levels." },
          });
        }
        if (stepCount === 2) {
          return JSON.stringify({
            ...
            action: "tool",
            tool: "calculator",
            reason: "Calculate total volume available: 75 + 45.",
            input: { expression: "75 + 45" },
          });
        }
        return JSON.stringify({
          workflow: "vision_calculation",
          action: "final",
          reason: "Visual math complete.",
          answer: "The primary tank is at 75% and reserve at 45%. Combined total is 120%.",
        });
      });

      const mockVisionClient = async () => "Gauge values: Primary Tank = 75%, Reserve Tank = 45%.";

      const res = await runWorkflowTask({
        message: "Read the levels from the gauges in this photo and calculate the sum.",
        userId: "test-eng-1",
        images: [SAMPLE_IMAGE_BASE64],
        options: { visionClient: mockVisionClient },
      });
      ...
      assert.strictEqual(res.steps[1].toolName, "calculator");
      assert.strictEqual(res.workflow?.type, WORKFLOW_TYPES.VISION_CALCULATION);
      assert.ok(res.response.includes("120%"));
      pass("Test 15 passed: Vision + Calculator task chains Vision → Calculator with vision_calculation workflow");
    }

    // ───────────────────────────────────────────────────────────────────────────
    // TEST 16: Endpoint Verification — POST /agent executes Simple Agent
    // ───────────────────────────────────────────────────────────────────────────
    console.log("\n--- [Test 16] Endpoint Verification: POST /agent ---");
    {
      workflowGraphService.setAgentBrainLlmClient(async () => {
        return JSON.stringify({
          ...
          answer: "Hello from the simple agent.",
        });
      });
      ...
      await createAgentTask({ body: { message: "Hello simple agent" } }, mockRes);
      assert.strictEqual(statusCode, 200);
      assert.strictEqual(responseBody.success, true);
      assert.strictEqual(responseBody.framework, "langgraph");
      assert.ok(responseBody.response.includes("simple agent"));
      pass("Test 16 passed: POST /agent properly invokes the Simple Agent returning framework: langgraph");
    }

    // ───────────────────────────────────────────────────────────────────────────
    // TEST 17: Endpoint Verification — POST /workflow executes 6-Workflow Agent
    // ───────────────────────────────────────────────────────────────────────────
    console.log("\n--- [Test 17] Endpoint Verification: POST /workflow ---");
    {
      workflowGraphService.setAgentBrainLlmClient(async () => {
        return JSON.stringify({
          workflow: "general",
          action: "final",
          reason: "Direct response from workflow agent.",
          answer: "Hello from the 6-workflow agent.",
        });
      });
      ...
      await createWorkflowTask({ body: { message: "Hello workflow agent" } }, mockRes);
      assert.strictEqual(statusCode, 200);
      assert.strictEqual(responseBody.success, true);
      ...
      assert.ok(responseBody.response.includes("6-workflow agent"));
      pass("Test 17 passed: POST /workflow properly invokes the 6-Workflow Agent returning framework: langgraph-workflows and workflow metadata");
    }

    // ───────────────────────────────────────────────────────────────────────────
    // TEST 18: Streaming Verification — POST /agent Streaming
    // ───────────────────────────────────────────────────────────────────────────
    console.log("\n--- [Test 18] Streaming Verification: POST /agent ---");
    {
      agentGraphService.setAgentBrainLlmClient(async () => {
        return JSON.stringify({
          action: "final",
          reason: "Stream test answer.",
          answer: "Streaming agent test completed.",
        });
      });
      ...
      await createAgentTask({ body: { message: "Test streaming", stream: true } }, mockSseRes);
      assert.ok(sseChunks.length > 0, "Must write SSE chunks");
      const fullSse = sseChunks.join("");
      assert.ok(fullSse.includes("agent_start"), "Must emit agent_start event");
      assert.ok(fullSse.includes("agent_complete"), "Must emit agent_complete event");
      assert.ok(fullSse.includes("Streaming agent test completed"), "Must contain final answer");
      pass("Test 18 passed: POST /agent streaming delivers SSE chunks (agent_start, reasoning, agent_complete)");
    }

    // ───────────────────────────────────────────────────────────────────────────
    // TEST 19: Streaming Verification — POST /workflow Streaming
    // ───────────────────────────────────────────────────────────────────────────
    console.log("\n--- [Test 19] Streaming Verification: POST /workflow ---");
    {
      workflowGraphService.setAgentBrainLlmClient(async () => {
        return JSON.stringify({
          workflow: "general",
          action: "final",
          reason: "Workflow stream test answer.",
          answer: "Streaming workflow test completed.",
        });
      });
      ...
      await createWorkflowTask({ body: { message: "Test workflow streaming", stream: true } }, mockSseRes);
      assert.ok(sseChunks.length > 0, "Must write SSE chunks");
      const fullSse = sseChunks.join("");
      assert.ok(fullSse.includes("workflow_start"), "Must emit workflow_start event");
      assert.ok(fullSse.includes("workflow_complete"), "Must emit workflow_complete event");
      assert.ok(fullSse.includes("Streaming workflow test completed"), "Must contain final answer");
      assert.ok(fullSse.includes("general"), "Must contain workflow metadata");
